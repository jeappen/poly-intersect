...
bool isValidInput(const std::string & s){ //, int *i
	if(isInteger(s)){ //isInteger(s)
		//*i=
		if(atoi(s.c_str())<0)
			return false;
		else
	...
	bool CheckPolyIntersect(Polygon poly1,Polygon poly2){ //General Polygon Intersection using sweep line
		...
		while(h<s1.size()){
			if(poly2.CheckPointStrictlyInPolygon(s1[h].p2)|| poly2.CheckPointStrictlyInPolygon(s1[h].p1))
				...
				cond=(poly2.CheckPointInPolygon(s1[h].p2) || poly2.CheckPointInPolygon(s1[h].p1));
			h++;
		}
		//}
		if(cond)
		...
		while(h<s2.size()){
			if(poly1.CheckPointStrictlyInPolygon(s2[h].p2)|| poly1.CheckPointStrictlyInPolygon(s2[h].p1))
				...
				cond=(poly1.CheckPointInPolygon(s2[h].p2) || poly1.CheckPointInPolygon(s2[h].p1));
			h++;
		}
		return cond;
