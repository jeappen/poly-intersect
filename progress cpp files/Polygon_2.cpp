...
protected:
	...
	bool CheckPolyIntersect(Polygon p1,Polygon p2){ //General Polygon Intersection
		...
		list< pair<Side,bool> > active_set; //bool is true if belong to poly1, false if belong to poly2
		...
		while(i != s1.end() || j != s2.end()){
			...
			// cout<<endl;p1.PrintSide(sInserted);cout<<endl;
			...
			for(k=active_set.begin();k!=active_set.end();k++){
				sTemp=k->first;
				// cout<<endl;p1.PrintSide(sTemp);cout<<endl;
				if(p1.doIntersect(sInserted.p1,sInserted.p2,sTemp.p1,sTemp.p2)){
					return true;
				}
			}
