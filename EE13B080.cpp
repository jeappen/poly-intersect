...
bool isInteger(const std::string & line)
{
	...
	char c;
	if (s >> c) {
	  return false;// Error, there was something past the number
	}
	return true;
}
	...
	void printCoords(){
  	...
  	protected:
  		...
  		int orientation(Point p, Point q, Point r)
  		{
  		    ...
  		    return (val > 0)? 1: 2; // anticlock or clock wise
  		}
  		...
  		bool CheckIfAntiClockwise(){ //Uses fact that integral of a poly along it's sides is negative if going anticlockwise
  			long sum=0;
  			int i;
  			for(i=0;i<x.size();i++){
  				if (i==(x.size()-1))
  		...
  		bool CheckPointStrictlyInPolygon(Point p){ //Checks if Point p is in Polygon. Lying on edge doesn't count.
  			if(p.x>=xmax || p.x<=xmin || p.y>=ymax || p.y<=ymin) //Is it outside bounding rectangle?
  				return false;
  			Point extreme={p.x,END_OF_Y}; //To make line x=(p.x) 
  			//Now to check number of intersections of x=(p.x) and the sides of the polygon.
...
protected:
	...
	bool CheckPolyIntersect(Polygon poly1,Polygon poly2){ //General Polygon Intersection using sweep line
		...
		while(h<s1.size()){
			if(poly2.CheckPointStrictlyInPolygon(s1[h].p2)|| poly2.CheckPointStrictlyInPolygon(s1[h].p1))
				return true;
			
				if(!(poly2.CheckPointInPolygon(s1[h].p2) || poly2.CheckPointInPolygon(s1[h].p1)))
					cond=false;
			h++;
		}
		if(cond)
		...
		h=0;
		while(h<s2.size()){
			if(poly1.CheckPointStrictlyInPolygon(s2[h].p2)|| poly1.CheckPointStrictlyInPolygon(s2[h].p1))
				return true;

				if(!(poly1.CheckPointInPolygon(s2[h].p2) || poly1.CheckPointInPolygon(s2[h].p1)))
					cond=false;
			h++;
		}
		return cond;
